...
from typing import Any, Awaitable, Callable, Dict, NoReturn, Optional, Text
...
class RestInput(InputChannel):
    ...
    def _extract_message(self, req: Request) -> Optional[Text]:
        message_type = req.json.get('type', None)
        if message_type == 'audio':
            message = transcribe_audio(req.json.get('message', None))
        else:
            message = req.json.get('message', None)
        return message.strip()

    def _extract_type(self, req) -> Optional[Text]:
