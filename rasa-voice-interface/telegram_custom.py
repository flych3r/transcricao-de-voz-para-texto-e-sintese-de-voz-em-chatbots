...
class TelegramOutput(TeleBot, OutputChannel):
    ...
    async def _extract_text_from_voice(self, msg: Message) -> Text:
        ...
        audio = BytesIO(audio.raw.data)
        AudioSegment.from_file(audio, format='ogg').export(audio, format='wav')
        audio.seek(0)
        encoded = b64encode(audio.getvalue())
    ...
    async def _create_voice_response(self, text: Text) -> BytesIO:
        ...
        audio = BytesIO(decode_string)
        import tempfile
        with tempfile.TemporaryFile() as fp:
            AudioSegment.from_file(audio, format='wav').export(fp, format='ogg')
            fp.seek(0)
            return fp.read()
